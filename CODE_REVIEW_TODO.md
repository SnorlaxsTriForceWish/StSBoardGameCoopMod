...
**File:** `src/main/java/BoardGame/characters/BGIronclad.java:39`
```java
...
grep -rn "this.title = \"" src/main/java/BoardGame/characters/
grep -rn "\.title = \"" src/main/java/BoardGame/
```

---

### 2. Potential Softlock in Neow Event Menu
**File:** `src/main/java/BoardGame/neow/BGNeowQuickStart.java:233`
```java
...
**Find Similar Issues:**
```bash
grep -rn "softlock" src/main/java/BoardGame/
grep -rn "might.*lock\|may.*lock\|could.*lock" src/main/java/BoardGame/
```

---

### 3. Multi-room Navigation Softlock
**File:** `src/main/java/BoardGame/multicharacter/patches/MultiDungeonTransitionPatches.java:45`
```java
...
**File:** `src/main/java/BoardGame/cards/BGColorless/BGMetamorphosis.java:44`
```java
...
**File:** `src/main/java/BoardGame/monsters/bgcity/BGSnakePlant.java:99`
```java
...
**File:** `src/main/java/BoardGame/monsters/bgbeyond/BGAwakenedOne.java:206`
```java
...
**File:** `src/main/java/BoardGame/potions/BGFairyPotion.java:58`
```java
//TODO!!!!
```
...
**File:** `src/main/java/BoardGame/thedie/TheDie.java:304,351`
```java
...
**Find Similar Issues:**
```bash
grep -rn "duplicate\|duplicated\|copy of\|same as" src/main/java/BoardGame/
```

---

### 9. Variable Naming Inconsistency
**File:** `src/main/java/BoardGame/powers/BGDoubleTapPower.java:30`
```java
...
grep -rn "Remaining\|Counter\|Count" src/main/java/BoardGame/powers/ | grep -v "amount"
```

---

### 10. Greedy Gremlins Power Condition Unknown
**File:** `src/main/java/BoardGame/powers/BGGreedyGremlinsPower.java:37`
```java
...
**File:** `src/main/java/BoardGame/powers/BGSneckoPower.java:55`
```java
...
**File:** `src/main/java/BoardGame/patches/OtherModsPatches.java:45`
```java
...
**Find Similar Issues:**
```bash
grep -rn "\"Board Game\"\|= \".*\".*//.*translat" src/main/java/BoardGame/
```

---

### 13. Font Size for Long Abilities
**File:** `src/main/java/BoardGame/thedie/TheDie.java:209`
```java
...
**File:** `src/main/java/BoardGame/cards/AbstractBGCard.java:606`
```java
...
**File:** `src/main/java/BoardGame/cards/AbstractBGCard.java:361,377`
```java
...
**File:** `src/main/java/BoardGame/savables/CurrentRoom.java:35`
```java
...
## Main BoardGame.java TODOs

**File:** `src/main/java/BoardGame/BoardGame.java`

| Line | Issue |
|------|-------|
| 66 | Game softlocks if BoardGame character started on new profile |
...
```bash
grep -rn "TODO\|FIXME\|XXX\|HACK\|BUG" src/main/java/BoardGame/
```

### Find Incomplete Implementations
```bash
grep -rn "not implemented\|incomplete\|stub\|placeholder" src/main/java/BoardGame/
```

### Find Potential Null Issues
```bash
grep -rn "// null\|//null\|might be null\|could be null" src/main/java/BoardGame/
```

### Find Hardcoded Values
```bash
# Hardcoded strings that might need localization
grep -rn "\.text = \"" src/main/java/BoardGame/
grep -rn "= \"[A-Z].*\";" src/main/java/BoardGame/

# Hardcoded numbers that might be magic numbers
grep -rn "= [0-9][0-9][0-9]" src/main/java/BoardGame/
```

### Find Commented-Out Code
```bash
grep -rn "^[[:space:]]*//[[:space:]]*[a-zA-Z].*(" src/main/java/BoardGame/ | head -50
```

### Find Empty/Stub Methods
```bash
grep -rA2 "public void\|public boolean\|public int" src/main/java/BoardGame/ | grep -B1 "^.*{$" | grep -A1 "^.*}$"
```
...
- [ ] Extract monster registration from `BoardGame.java` to registry class
- [ ] Extract event registration from `BoardGame.java` to registry class
