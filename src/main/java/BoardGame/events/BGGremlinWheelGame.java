...
public class BGGremlinWheelGame extends AbstractImageEvent {
    ...
    public void update() {
        ...
        if (this.bounceTimer == 0.0F && this.startSpin) {
            if (!this.buttonPressed) {
                this.buttonHb.cY = MathHelper.cardLerpSnap(
                    this.buttonHb.cY,
                ...
                if (
                    (this.buttonHb.hovered && InputHelper.justClickedLeft) ||
        ...
        if (this.startSpin && this.bounceTimer == 0.0F && this.buttonPressed) {
            ...
            if (this.animTimer > 0.0F) {
                ...
                this.wheelAngle = Interpolation.elasticIn.apply(
                    this.tmpAngle,
                    -180.0F,
                    this.animTimer / 3.0F
                );
                    ...
                    if (r != null) {
                        if (!alreadyUsedGamblingChip) {
                            if (buttonPressed == gamblingChipIndex) {
                                ...
                                this.result = AbstractDungeon.miscRng.random(0, 5);
                                this.resultAngle =
                                    this.result * 60.0F + MathUtils.random(-10.0F, 10.0F);
                                this.wheelAngle = 0.0F;
                                this.startSpin = true;
                                this.bounceTimer = 2.0F;
                                this.animTimer = 2.0F;
            ...
            case 3:
                ...
                AbstractDungeon.effectList.add(
                    new ShowCardAndObtainEffect(
                        (AbstractCard) AbstractDungeon.getCard(AbstractCard.CardRarity.CURSE),
                        Settings.WIDTH / 2.0F,
        ...
        if (this.buttonHb.hovered) {
            ...
            sb.setColor(
                new Color(
                    ...
                    1.0F,
                    1.0F,
                    (MathUtils.cosDeg((float) ((System.currentTimeMillis() / 5L) % 360L)) + 1.25F) /
                        3.5F
                )
            );
        }
    ...
    public void dispose() {
        ...
        if (this.buttonImg != null) this.buttonImg.dispose();
    }
}
