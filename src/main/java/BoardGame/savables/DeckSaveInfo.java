...
public class DeckSaveInfo
    implements CustomSavable<HashMap<DeckSaveInfo.DeckType, ArrayList<String>>> {
    ...
    public static void populateCardDeck(
        ArrayList<AbstractCard> destination,
        ArrayList<String> rewardDeck
    ) {
        destination.clear();
    ...
    public static void populateRelicDeck(
        ArrayList<AbstractRelic> destination,
        ArrayList<String> rewardDeck
    ) {
        destination.clear();
        for (String id : rewardDeck) {
    ...
    public void onLoad(HashMap<DeckType, ArrayList<String>> decks) {
        ...
        for (int i = 0; i < 4; i += 1) {
            AbstractBGDungeon.physicalRewardDecks.add(
                new CardGroup(CardGroup.CardGroupType.CARD_POOL)
            );
            AbstractBGDungeon.physicalRareRewardDecks.add(
                new CardGroup(CardGroup.CardGroupType.CARD_POOL)
            );
        }
        ...
        populateCardDeck(
            AbstractBGDungeon.physicalRewardDecks.get(0).group,
            decks.get(RED_REWARDS)
        );
        populateCardDeck(
            AbstractBGDungeon.physicalRareRewardDecks.get(0).group,
            decks.get(RED_RARES)
        );
        populateCardDeck(
            AbstractBGDungeon.physicalRewardDecks.get(1).group,
            decks.get(GREEN_REWARDS)
        );
        populateCardDeck(
            AbstractBGDungeon.physicalRareRewardDecks.get(1).group,
            decks.get(GREEN_RARES)
        );
        populateCardDeck(
            AbstractBGDungeon.physicalRewardDecks.get(2).group,
            decks.get(BLUE_REWARDS)
        );
        populateCardDeck(
            AbstractBGDungeon.physicalRareRewardDecks.get(2).group,
            decks.get(BLUE_RARES)
        );
        populateCardDeck(
            AbstractBGDungeon.physicalRewardDecks.get(3).group,
            decks.get(PURPLE_REWARDS)
        );
        populateCardDeck(
            AbstractBGDungeon.physicalRareRewardDecks.get(3).group,
            decks.get(PURPLE_RARES)
        );
