...
public class BottledPlaceholderRelic
    extends CustomRelic
    implements CustomBottleRelic, CustomSavable<Integer> {
    ...
    private static final Texture OUTLINE = TextureLoader.getTexture(
        makeRelicOutlinePath("BottledPlaceholder.png")
    );
    ...
    public void onEquip() {
        ...
        AbstractDungeon.gridSelectScreen.open(
            group,
            1,
            DESCRIPTIONS[3] + name + DESCRIPTIONS[2],
            false,
            false,
            false,
            false
    ...
    public void onUnequip() {
        // 1. On unequip
        if (card != null) {
            // If the bottled card exists (prevents the game from crashing if we removed the bottled card from our deck for example.)
    ...
    public void onUseCard(AbstractCard targetCard, UseCardAction useCardAction) {
        ...
        for (
            ...
            if (BottledPlaceholderField.inBottledPlaceholderField.get(card)) {
                // Check if our SpireField matches said card
                ...
                if (AbstractDungeon.player.hand.size() < BaseMod.MAX_HAND_SIZE) {
                    // If your hand isn't full
                    if (AutoplayField.autoplay.get(card)) {
                        // If the card auto-plays - auto play it
                        AbstractDungeon.actionManager.addToBottom(
                            new AutoplayCardAction(card, AbstractDungeon.player.hand)
                        );
                    }
                    ...
                    for (AbstractRelic r : AbstractDungeon.player.relics) {
                        // And if you have any relics that trigger on card draw - trigger them
                        r.onCardDraw(card);
                    }
                } else {
                    // If your hand IS full - create a single "My hand is full!" dialogue and move the card to the discard pile instead
                    if (!fullHandDialog) {
    ...
    public void setDescriptionAfterLoading() {
        this.description =
            DESCRIPTIONS[1] + FontHelper.colorString(card.name, "y") + DESCRIPTIONS[2];
        this.tips.clear();
