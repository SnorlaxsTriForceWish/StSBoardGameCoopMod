...
public class BGAcidSlime_L extends AbstractBGMonster implements BGDamageIcons, DieControlledMoves {
    ...
    private static final MonsterStrings monsterStrings =
        CardCrawlGame.languagePack.getMonsterStrings("AcidSlime_L");
    ...
    public BGAcidSlime_L(float x, float y, boolean hard) {
        ...
        loadAnimation(
            "images/monsters/theBottom/slimeL/skeleton.atlas",
    ...
    public void takeTurn() {
        switch (this.nextMove) {
            ...
            case 3: {
                ...
                for (
                    int i = 0;
                    slimesSpawned < slimesPerSpawn && i < this.mediumslimes.length;
                    i++
                ) {
                    if (this.mediumslimes[i] == null || this.mediumslimes[i].isDeadOrEscaped()) {
                        ...
                        AbstractDungeon.actionManager.addToBottom(
                            (AbstractGameAction) new SpawnMonsterAction(slimeToSpawn, false)
                        );
                        slimesSpawned++;
                    }
                }
    ...
    public void dieMove(int roll) {
        ...
        else if (move == 'X') setMove("Summon", (byte) 3, AbstractMonster.Intent.UNKNOWN);
        else if (move == 'S') setMove(
            WOUND_NAME,
            (byte) 1,
